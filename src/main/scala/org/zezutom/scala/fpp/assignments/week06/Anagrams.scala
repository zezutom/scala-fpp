package org.zezutom.scala.fpp.assignments.week06
...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    def extract(occ: Occurrences, res: Occurrences = List()): Occurrences = occ match {
      case Nil => Nil
      case x :: xs => res ::: List(x) ::: extract(if (x._2 > 1) (x._1, x._2 - 1) :: xs else xs)
    }
    def combine(occ: Occurrences): List[Occurrences] = occ match {
      ...
      case _ => {
        ...
        matrix.flatten map { _.sorted } distinct
      }
    }
    if (occurrences.isEmpty) List(Nil)
    else {
      val extracted = extract(occurrences)
      combine(extracted) ::: (extracted map { x: (Char, Int) => List(x) }) ::: List(List())
    }
  }
