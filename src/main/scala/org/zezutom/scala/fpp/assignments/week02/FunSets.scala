...
object FunSets {
  ...
  def forall(s: Set, p: Int => Boolean): Boolean = {
    def iter(a: Int): Boolean = {
      if (a > bound) false
      	...
      	if (contains(s, a + 1)) p(a) && iter(a + 1)
      	else p(a)
  ...
  def exists(s: Set, p: Int => Boolean): Boolean = {
  	def iter(a: Int): Boolean = {
  		if (a > bound) false
  		  ...
  		  if (contains(s, a + 1)) forall(singletonSet(a), p) || iter(a + 1)
  		  else forall(singletonSet(a), p)
  		else iter(a + 1)
  	}
  	iter(-bound)
  }
  ...
  def map(s: Set, f: Int => Int): Set = {
  	...
  	def iter(a: Int, r: Set): Set = {
  		if (a > bound) r
  		else if (contains(s, a)) union(singletonSet(f(a)), iter(a + 1, r))
  		else iter(a + 1, r)
  	}
  	iter(-bound, Set())
  }
